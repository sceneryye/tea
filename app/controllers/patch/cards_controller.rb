...
class Patch::CardsController < ApplicationController
	...
	def confirm_lost
		...
		if @user.check_sms(sms_code)
			...
			if operator == "user"
				begin
					@sms_log ||= Logger.new('log/sms.log')
					text = "您购买的佐康VIP卡#{@card.no}已被#{mask(@card.member_card.user_tel)}挂失,如有疑问请致电400-826-4568[佐康]"
					if Sms.send(@card.member_card.buyer_tel,text)
						tel = @card.member_card.buyer_tel
						@sms_log.info("[#{@user.login_name}][#{Time.now}][#{tel}]#{text}")
					end
					text = "您的佐康VIP卡#{@card.no}已锁定,如有疑问请致电客服400-826-4568[佐康]"
					if Sms.send(@card.member_card.user_tel,text)
						tel = @card.member_card.user_tel
						@sms_log.info("[#{@user.login_name}][#{Time.now}][#{tel}]#{text}")
					end
	...
	def confirm_cancel
		...
		if @user.check_sms(sms_code)
			...
			if operator == "buyer"
				begin
					@sms_log ||= Logger.new('log/sms.log')
					text = "您购买的佐康VIP卡#{@card.no}已被重新启用,如有疑问请致电客服400-826-4568[佐康]"
					if Sms.send(@card.member_card.buyer_tel,text)
						tel = @card.member_card.buyer_tel
						@sms_log.info("[#{@user.login_name}][#{Time.now}][#{tel}]#{text}")
					end
					text = "您的佐康VIP卡#{@card.no}已被重新启用,如有疑问请致电客服400-826-4568[佐康]"
					if Sms.send(@card.member_card.user_tel,text)
						tel = @card.member_card.user_tel
						@sms_log.info("[#{@user.login_name}][#{Time.now}][#{tel}]#{text}")
					end
			...
			if operator == "user"
				begin
					@sms_log ||= Logger.new('log/sms.log')
					text = "您购买的佐康VIP卡#{@card.no}已被#{mask(@card.member_card.buyer_tel)}启用,如有疑问请致电400-826-4568[佐康]"
					if Sms.send(@card.member_card.buyer_tel,text)
						tel = @card.member_card.buyer_tel
						@sms_log.info("[#{@user.login_name}][#{Time.now}][#{tel}]#{text}")
					end
	...
	def update_buyer_tel
		...
		if  @user.check_sms(@sms_code)
			...
			begin
				@sms_log ||= Logger.new('log/sms.log')
				text = "您购买的佐康VIP卡#{@card.no}已更改购卡人的联系方式,如有疑问请致电400-826-4568[佐康]"
	...
	def update_user_tel
		...
		if  @user.check_sms(@sms_code)
			...
			if @card.use_status
				begin
					@sms_log ||= Logger.new('log/sms.log')
					text = "您的佐康VIP卡#{@card.no}联系方式已改为#{mask(@user_tel)}如有疑问请致电400-826-4568[佐康]"
				...
				begin
					@sms_log ||= Logger.new('log/sms.log')
					text = "您购买的佐康VIP卡#{@card.no}已更改指定用卡人的联系方式,如有疑问请致电400-826-4568[佐康]"
					if Sms.send(@card.member_card.buyer_tel,text)
						tel = @card.member_card.buyer_tel
						@sms_log.info("[#{@user.login_name}][#{Time.now}][#{tel}]#{text}")
					end
